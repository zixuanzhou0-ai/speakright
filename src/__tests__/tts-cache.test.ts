...
describe("TTS cache key format", () => {
  ...
  it("separates identical audio requests by provider, voice, and model", () => {
    ...
    const miniMax = buildCacheKey(
      "hello",
      "minimax:English_expressive_narrator:speech-2.8-turbo",
      1,
    );
    const mimo = buildCacheKey("hello", "mimo:Mia:mimo-v2.5-tts:prompt-v1", 1);

    expect(new Set([elevenLabs, hermes, otherModel, miniMax, mimo]).size).toBe(
      5,
    );
  });
