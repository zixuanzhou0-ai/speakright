...
describe("useTtsAligned", () => {
  beforeEach(() => {
    ...
    mocks.miniMaxTtsAligned.mockResolvedValue({
      audioBlob: new Blob([new Uint8Array([10, 11, 12])], {
        type: "audio/mpeg",
      }),
      wordTimings: [
        { word: "MiniMax", start: 0, end: 0.4 },
        { word: "audio.", start: 0.42, end: 0.8 },
      ],
      alignmentOutcome: "matched",
    });
    mocks.mimoTts.mockResolvedValue(
      new Blob([new Uint8Array([13, 14, 15])], { type: "audio/wav" }),
    );
  ...
  it("uses MiniMax real word timings and provider-isolated persistent cache", async () => {
    ...
    await act(async () => {
      await result.current.speak("MiniMax audio.", {
        languageId: "en-US",
        speed: 0.9,
      });
    });
  ...
  it("uses MiMo sentence playback and versions its prompt cache identity", async () => {
