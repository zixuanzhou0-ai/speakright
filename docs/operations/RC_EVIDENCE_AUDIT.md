...
| Words, phrases, sentences, IPA, and examples are centered, wrap in full, avoid ellipsis/truncation, and use accurate playback labels | `src/lib/practice-text-presentation.ts`, `src/components/phoneme/phoneme-study-card.tsx`, `src/components/drill/drill-feedback.tsx`, `src/components/layout/sidebar-phoneme-list.tsx`, `src/components/audio/read-along-text.tsx`, `src/components/sentences/sentence-input-card.tsx`, `src/components/scoring/word-highlight.tsx`, `src/components/settings/language-config-card.tsx`, `src/components/settings/usage-monitor.tsx`, `src/__tests__/practice-text-presentation.test.ts`, `src/__tests__/phoneme-study-card.test.tsx`, `src/__tests__/drill-feedback.test.tsx`, `src/__tests__/sidebar-phoneme-list.test.tsx`, `src/__tests__/desktop-preflight-ui-smoke.test.ts`, `scripts/desktop-ui-smoke.mjs`; non-English detail A/B playback buttons label rule sentences as rule/sentence playback instead of saying word playback, long Cyrillic rule text remains centered/wrap-ready/untruncated, and failed drill feedback actions wrap instead of forcing a narrow-window horizontal row |
...
npm.cmd run audio:parity:dry-run
  ...
  Russian 918 normalized lookup items / 0 missing; bundled Russian files
  remain 460 x 2 because punctuation variants can share one local clip; no
  ElevenLabs calls
