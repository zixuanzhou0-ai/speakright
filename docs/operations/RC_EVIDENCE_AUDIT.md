...
| Public repository has basic open-source governance, asset-boundary documentation, release-script guardrails, and a tracked-file secret-format guard | `LICENSE`, `CONTRIBUTING.md`, `SECURITY.md`, `THIRD_PARTY_NOTICES.md`, `.env.example`, `.github/ISSUE_TEMPLATE/*.md`, `.github/pull_request_template.md`, `README.md`, `package.json`, `src/__tests__/open-source-readiness.test.ts`; source code is MIT-licensed while bundled media is explicitly not automatically relicensed, public developer/release npm scripts remain explicit, routine validation scripts do not call audio generation, and tracked text files are checked for obvious real private-key/API-token formats without printing matched secret text |
