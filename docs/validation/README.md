...
| GitHub release verification | an observed Release ID, peeled tag commit, workflow run, asset set, checksums, and signed-out URLs matched the release contract at a dated snapshot | adoption, download-to-user conversion, future tag immutability, code signing, or learning efficacy |
...
The immutable v1.1.0 pre-publication source, runtime, CI, and owner-controlled
gate ledger is
[`V1.1.0_RELEASE_CANDIDATE.md`](V1.1.0_RELEASE_CANDIDATE.md). It remains a
candidate snapshot and is not rewritten after publication.

The separate
[`V1.1.0_RELEASE_VERIFICATION.md`](V1.1.0_RELEASE_VERIFICATION.md) is the only
place that should certify the observed Browser Stable and unsigned Desktop
